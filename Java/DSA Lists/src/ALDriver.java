...
public class ALDriver {
    ...
    private static ListArrayListBasedPlus alPlus;
    /** Reader used to receive input**/
    private static BufferedReader br = new BufferedReader(new InputStreamReader(System.in));


    public static void main(String[] args) throws NumberFormatException, IOException {
    	alPlus = new ListArrayListBasedPlus();
        printMenu();
    }
	...
	if (index <= alPlus.size()){
		alPlus.add(index,item);		
	...
	if (alPlus.size() > 0){
        	System.out.printf("\n\tList of size %d has the following items : %s",alPlus.size(),alPlus.toString());
	}	// end if
	else{
